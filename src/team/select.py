...
class UpdateXVSelector(XVSelector):
    ...
    def __init__(self, player_df, season, initial_xi, budget=1000):
        self.inital_xi_added = False
        self.initial_xi = initial_xi
        super().__init__(player_df, season, budget)
        self.changes = self._create_decision_var("change_", self.player_df)
    ...
    # maybe this could just be done as a decorator...?
    # likely there's a better way to reduce the code
    def _add_changes_constraint(self, prob, max_changes):
        pass
        # prob += sum()# TODO: this should be closer to how i implement player limits
        # prob += sum(self.changes) <= max_changes
        # prob += (
        #     lpSum(
        #         self.changes[i]
        #         for i in range(len(self.player_df))
