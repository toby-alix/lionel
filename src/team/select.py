...
class Optimiser:
    ...
    def _pick_xv(self):
        players = [LpVariable("player_" + str(i), cat="Binary") for i in self.player_df.index] 
        captain_decisions = [LpVariable("captain_" + str(i), cat="Binary") for i in self.player_df.index]
        ...
        if self.max_var == "value_fixture":
            prob += lpSum((players[i] + captain_decisions[i]) * self.points_weighted[i]  for i in range(len(self.player_df))) 
        else: 
            prob += lpSum((players[i] + captain_decisions[i]) * self.total_points[i]  for i in range(len(self.player_df))) 

        # Constraints
        prob += sum(players) ==  15
        prob += lpSum(players[i] * self.player_df.value[self.player_df.index[i]] for i in range(len(self.player_df))) <= self.budget
        prob += sum(captain_decisions) == 1

        for pos in ['GK', 'DEF', 'MID', 'FWD']:
            prob += lpSum(players[i] for i in range(len(self.player_df)) if self.positions[i] == pos) <= self.POS_CONSTRAINTS['XV'][pos]

        for club in self.teams:
            prob += lpSum(players[i] for i in range(len(self.player_df)) if self.teams[i] == club) <= 3 # Club Limit
    ...
    def pick_xi(self):
        ...
        for pos in ['GK', 'DEF', 'MID', 'FWD']:
            prob += lpSum(players[i] for i in range(len(team)) if self.positions[i] == pos) >= self.POS_CONSTRAINTS['XI'][pos][0]
            prob += lpSum(players[i] for i in range(len(team)) if self.positions[i] == pos) <= self.POS_CONSTRAINTS['XI'][pos][1]
